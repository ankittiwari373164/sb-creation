...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async (userId: string) => {
    try {
      // Fetch Orders and Wishlist count at the same time
  ...
  return (
    <div className="min-h-screen bg-[#fffdfa] py-16 px-4 md:px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
            <div className="text-center md:text-left">
              <h1 className="text-4xl md:text-5xl font-serif text-[#0F2C3E]">
                Hello, <span className="italic font-light text-[#D4AF37]">{user?.user_metadata?.full_name?.split(' ')[0] || 'Member'}</span>
              </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            ...
            <div className="bg-white rounded-[2rem] p-8 border border-gray-100 shadow-sm flex items-center gap-6">
              <div className="w-14 h-14 rounded-2xl bg-[#FAF9F6] flex items-center justify-center text-[#0F2C3E]">
                <Package size={24} />
              </div>
              <div>
            ...
            {/* 📍 Active Orders */}
            <div className="bg-white rounded-[2rem] p-8 border border-gray-100 shadow-sm flex items-center gap-6">
              <div className="w-14 h-14 rounded-2xl bg-[#f0f9ff] flex items-center justify-center text-blue-500">
                <Truck size={24} />
              </div>
              <div>
            ...
            {/* 💳 Saved Details */}
            <div className="bg-[#0F2C3E] rounded-[2rem] p-8 text-white flex items-center gap-6 shadow-xl">
              <div className="w-14 h-14 rounded-2xl bg-white/10 flex items-center justify-center text-[#D4AF37]">
                <Sparkles size={24} />
              </div>
              <div>
                <p className="text-[10px] font-bold uppercase text-[#D4AF37]">Points</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-[2.5rem] p-8 md:p-10 shadow-sm border border-gray-100">
                ...
                {orders.length === 0 ? (
                  <div className="text-center py-20 bg-[#FAF9F6] rounded-3xl border-2 border-dashed border-gray-100">
                    ...
                    <Link href="/shop" className="text-[#D4AF37] font-bold text-xs uppercase mt-4 inline-block">Start Shopping</Link>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {orders.slice(0, 5).map((order) => {
                      ...
                      return (
                        <div key={order.id} className="flex flex-col md:flex-row md:items-center justify-between p-6 rounded-3xl bg-[#FAF9F6] border border-transparent hover:border-gray-200 transition-all gap-4">
                          <div className="flex items-center gap-5">
                             <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm">
                                <Package size={18} className="text-[#0F2C3E]" />
                             </div>
                             <div>
                          ...
                          <div className="flex items-center justify-between md:justify-end gap-8">
                             ...
                             <Link href={`/track-order?id=${order.id}`}>
                                <button className="p-2 bg-white rounded-full text-gray-400 hover:text-[#0F2C3E] shadow-sm"><ArrowRight size={16}/></button>
                             </Link>
                          </div>
                        </div>
                      )
            ...
            <div className="space-y-6">
               <div className="bg-[#FAF9F6] p-10 rounded-[2.5rem] border border-[#D4AF37]/10">
                  <h3 className="text-xl font-serif text-[#0F2C3E] mb-6">Quick Actions</h3>
                  <div className="grid grid-cols-1 gap-3">
                     <Link href="/wishlist" className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-gray-100 hover:border-[#db2777] transition-all group">
                     ...
                     <Link href="/track-order" className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-gray-100 hover:border-[#0F2C3E] transition-all group">
                        <Truck size={18} className="text-[#0F2C3E]" />
                        <span className="text-xs font-bold uppercase tracking-widest text-[#0F2C3E]">Track Package</span>
                     </Link>
                     <button className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-gray-100 hover:border-[#D4AF37] transition-all group">
               ...
               <div className="bg-white p-10 rounded-[2.5rem] border border-gray-100">
                  <MessageCircle className="text-[#D4AF37] mb-4" size={28} />
                  <h3 className="text-lg font-serif text-[#0F2C3E] mb-2">Need Help?</h3>
                  <p className="text-xs text-gray-500 leading-relaxed mb-6">Questions about your order or bangle size? We are here to help.</p>
                  <Link href="/contact" className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#0F2C3E] bg-[#FAF9F6] px-6 py-3 rounded-full hover:bg-[#0F2C3E] hover:text-white transition-all inline-block">
                    Contact Support
                  </Link>
               </div>
            </div>
